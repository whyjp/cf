...
```pwsh
# 단일 카테고리 2 페이지
uv run python -m tkcp_crawl.cli pull --site-tp BB000 --max-pages 2

# 전체 카테고리 풀 풀 (~17 분)
uv run python -m tkcp_crawl.cli pull

# 디버그용 단일 페이지 dump
uv run python -m tkcp_crawl.cli inspect-page 1 --site-tp BB000

# data/camps.jsonl → data/camps.csv 일괄 재생성
uv run python -m tkcp_crawl.cli dedup-csv
```
...
robots.txt 전체 허용. 자세한 내용은 `.ShipofTheseus/tkcp-crawl/intent/probes/FINDINGS.md`.
...
| TKCP_DELAY_MAX | `3.0` |
| TKCP_LOG_LEVEL | `INFO` |
| TKCP_MAX_PAGES_DEFAULT | `600` |
| TKCP_LIVE | (1 = smoke test 활성) |
...
- 후속 분리 시 본 패키지의 모듈 매핑:
  ...
  - `crawlers/tkcp/` ← adapter / crawler / cli (사이트 종속)
