...
| **T24 HeuristicNegationExtractor** | Add **temperature lexicon** alongside `NEG_TOKENS`. The lexicon is **NOT hand-curated** — see new T23.5 below. |
| **NEW T23.5 Derive intensifier lexicon (agent-driven)** | Run a one-shot agent analysis on the actual review corpus (`data/reviews/*.json` — currently ~89 camps × top-3 reviews ≈ 267 reviews; expanding as P1 background fetches). Agent receives sentences grouped by review `totalScore` (≥80 → positive cluster; ≤30 → negative cluster; middle bucket discarded). Agent outputs `domain/intensifier_lexicon.py` with two constants `INTENSIFIER_POSITIVE` and `INTENSIFIER_NEGATIVE` (tuples of Korean tokens). Re-runnable as new reviews arrive. The output file is committed for reproducibility but regenerated on demand. |
